...
**Explore the conservation tiers using our interactive platform:** <https://mesic.app/>
...
├── scripts/
│   ├── 01_GEE_Biome_Stats.js               # Google Earth Engine: Biome-wide persistence analysis
...
│   ├── 04_R_Sensitivity_Analysis.R          # R Script: NDVI threshold sensitivity analysis
│   └── outputs/
│       ├── figures/    # Generated figures (gitignored)
│       ├── tables/     # Generated summary tables (gitignored)
│       └── spatial/    # Generated shapefiles (gitignored)
└── README.md           # This file
```
...
1.  **Clone this Repository:** Download the code to your computer. `bash     git clone https://github.com/krisMueller/Beyond_Uplands_Mesic_Frameworks.git`
...
3.  **Verification:** Your `data/input_data/` folder should now contain:
    ...
    -   `YYYYMMDD_valleyBottom_persistence_stratifiedSample_75k.csv` (required for script 04)
...
All outputs are written to `scripts/outputs/` and are gitignored (not stored on GitHub). Filenames are prefixed with the run date.

**1. Spatial Data (`scripts/outputs/spatial/`)** \* `_Classified_Watersheds_Tiered.shp`: The final HUC12 shapefile containing the assigned `Tier` and `MgmtAction` for every watershed in the biome.

**2. Figures (`scripts/outputs/figures/`)** \* `_Biome_Wide_Tier_Map.png`: A map of the western U.S. showing the three conservation tiers. \* `_Overall_Priority_Summary.png`: Bar chart of the proportion of watersheds in each tier. \* `_State_Ownership_Comparison.png`: Bar chart comparing Public vs. Private restoration opportunities by state. \* `_State_Summary_[StateName].png`: Individual dashboard reports for each state (e.g., MT, WY, NV). \* `_Combined_Summary_Figure.png`: Multi-panel summary figure.

**3. Tables (`scripts/outputs/tables/`)** \* `_Overall_Summary_Table.csv`: Summary statistics of acreage and watershed counts by Tier.

### Sensitivity Analysis

`04_R_Sensitivity_Analysis.R` validates the NDVI threshold choice by comparing persistence estimates across alternative thresholds. Run this script independently from the `scripts/` directory after downloading the stratified sample CSV (`YYYYMMDD_valleyBottom_persistence_stratifiedSample_75k.csv`) from Zenodo.
